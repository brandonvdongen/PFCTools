...
namespace PFCTools.Utils
{
    ...
    {
        private AnimatorLayerCopy instance;
        ...
        [MenuItem("PFCTools/Deprecated/Animator merge tools")]
                    ...
                    foreach (VRCExpressionParameters.Parameter parameter in ExpressionSource.parameters)
                    {
                        if (ExpressionTarget.FindParameter(parameter.name) == null)
                        {
                            ...
                            if (GUILayout.Button("Add", GUILayout.Width(80)))
                            {
                                ...
                                ExpressionTarget.parameters = tempList.ToArray();
                                EditorUtility.SetDirty(ExpressionTarget);
        ...
        private AnimatorControllerParameter getParameter(AnimatorController animator, string name)
        {
        ...
        private void ensureParameterExists(AnimatorController animator, string name)
        {
        ...
        private void addParameterToTarget(string name, AnimatorControllerParameterType type)
        {
        ...
        private void addLayerToTarget(string name, AnimatorControllerLayer copyFrom)
        {
            ...
            newLayer.name = copyFrom.name;
            newLayer.defaultWeight = copyFrom.defaultWeight;
            newLayer.stateMachine = newStateMachine;
            newLayer.stateMachine.anyStatePosition = copyFrom.stateMachine.anyStatePosition;
            newLayer.stateMachine.exitPosition = copyFrom.stateMachine.exitPosition;
            newLayer.stateMachine.entryPosition = copyFrom.stateMachine.entryPosition;

            Dictionary<int, StateData> states = new Dictionary<int, StateData>();

            foreach (ChildAnimatorState copyFromState in copyFrom.stateMachine.states)
            {
                StateData stateData = new StateData();

                AnimatorState newState = newStateMachine.AddState(copyFromState.state.name, copyFromState.position);
                ...
                CopyFields<AnimatorState>(copyFromState.state, newState);

                if (!String.IsNullOrEmpty(copyFromState.state.cycleOffsetParameter)) { ensureParameterExists(AnimatorControllerTarget, copyFromState.state.cycleOffsetParameter); }
                if (!String.IsNullOrEmpty(copyFromState.state.mirrorParameter)) { ensureParameterExists(AnimatorControllerTarget, copyFromState.state.mirrorParameter); }
                if (!String.IsNullOrEmpty(copyFromState.state.speedParameter)) { ensureParameterExists(AnimatorControllerTarget, copyFromState.state.speedParameter); }
                if (!String.IsNullOrEmpty(copyFromState.state.timeParameter)) { ensureParameterExists(AnimatorControllerTarget, copyFromState.state.timeParameter); }


                if (copyFromState.state.motion is AnimationClip)
                {
                    newState.motion = copyFromState.state.motion;
                }
                if (copyFromState.state.motion is BlendTree)
                {
                    BlendTree copyTree = (BlendTree)copyFromState.state.motion;
                    BlendTree blendTree = new BlendTree();
                    CopyFields<BlendTree>(copyFromState.state.motion as BlendTree, blendTree);
                ...
                stateData.copyState = copyFromState.state;
                stateData.InstanceID = copyFromState.state.GetInstanceID();

                stateData.state.behaviours = copyFromState.state.behaviours;
        ...
        ;
            ...
            foreach (AnimatorStateTransition copyTransition in copyFrom.stateMachine.anyStateTransitions)
            {
            ...
            foreach (AnimatorTransition copyTransition in copyFrom.stateMachine.entryTransitions)
            {
                if (copyTransition.destinationState != null)
                {
            ...
            newLayer.stateMachine.defaultState = states[copyFrom.stateMachine.defaultState.GetInstanceID()].state;

            AnimatorControllerTarget.AddLayer(newLayer);
            AssetDatabase.SaveAssets();
        }

        private static void CopyFields<T>(T source, T target)
        {
